...
def chek_if_num(text: str):
    if text == "." or text.isdigit() or text == "":
        return True
    else:
        return False
...
class YoutubeDownloaderGUI(tk.Tk):
    def __init__(self):
        ...
        self.trim_button = tk.Button(
            self, text="Trim Video", command=self.trim_video, state="disabled")

        self.progress_bar_container = tk.Frame(self)

        self.output_path_label = tk.Label(self, text="Output file path:")
    ...
    def trim_video(self):
        ...
        trimmedvideo.write_videofile(f"./videos/{self.file_name}-trimmed.mp4")
